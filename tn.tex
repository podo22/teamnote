...
\begin{document} \begin{multicols*}{3}
  ...
  \Algorithm{Binomial Coefficient}{}{Init: $O(MAXN)$, Query: $O(1)$}{cpp}{source/1. Math/Binomial Coefficient.cpp}{}
  %\Algorithm{Binomial Coefficient}{}{}{cpp}{source/1. Math/Binomial Coefficient(pe).cpp}{}
  ...
  \Algorithm{Gaussian Elimination}{}{}{cpp}{source/1. Math/Gaussian Elimination.cpp}{}
  ...
  \Algorithm{Sparse Table}{}{}{cpp}{source/2. Data structure/Sparse Table.cpp}{}
  \Algorithm{Mo's Algorithm}{}{}{cpp}{source/2. Data structure/Mos.cpp}{}
  ...
  \Algorithm{ETT}{}{}{cpp}{source/3. Graph/ETT.cpp}{}
  ...
  \begin{itemize}
    ...
    \item 2차원 누적합 / imos ($i$ = 행, $j$ = 열, 1-based)\\
    누적합 생성: $S[i][j] = S[i-1][j] + S[i][j-1] - S[i-1][j-1] + A[i][j]$\\
    구간합 $[i_1,j_1]\sim[i_2,j_2]$: $S[i_2][j_2] - S[i_1-1][j_2] - S[i_2][j_1-1] + S[i_1-1][j_1-1]$\\
    imos ($+v$), 부호 패턴 $+,-,-,+$:\\
    $d[i_1][j_1] \mathrel{+}= v,\ \ d[i_2+1][j_1] \mathrel{-}= v,\ \ d[i_1][j_2+1] \mathrel{-}= v,\ \ d[i_2+1][j_2+1] \mathrel{+}= v$\\
    (imos는 마지막에 누적합 2번 적용: 행 방향, 열 방향)
    \item 순열 swap 관련\\
    - 버블소트 최소 swap = inversion 개수\\
    - 순열 정렬 최소 swap = $N - (\text{사이클 개수})$
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \begin{enumerate}
      ...
      \item $v_i$가 F일 때 비용 발생: T에서 $i$로 가는 비용 간선
      \item $v_i$가 T이고 $v_j$가 F일 때 비용 발생: $i$에서 $j$로 가는 비용 간선
      \item $v_i \ne v_j$일 때 비용 발생: $i$에서 $j$로, $j$에서 $i$로 가는 비용 간선
      \item $v_i$가 T면 $v_j$도 T여야 함($v_i \implies v_j$): $i$에서 $j$로 가는 무한 간선
      \item $v_i$가 F면 $v_j$도 F여야 함($v_j \implies v_i$): $j$에서 $i$로 가는 무한 간선
      \item $v_i$가 무조건 T/F: T에서 $i$ / $i$에서 F로 가는 무한 간선
      \item $v_i = v_j$여야 함: $i \to j$, $j \to i$ 양방향 무한 간선
      \item $v_i \ne v_j$여야 함: 이분 그래프만 가능. $v'_j = \neg v_j$, $v_i = v'_j$로 치환 ($i \leftrightarrow j$ 무한 간선)
      \item $\neg(v_i \land v_j)$ (둘 다 T 금지): 이분 그래프만 가능. $v'_j = \neg v_j$, $v_i \implies v'_j$ ($i \to j$ 무한 간선)
      \item $v_i \lor v_j$ (둘 다 F 금지): 이분 그래프만 가능. $v'_j = \neg v_j$, $v'_j \implies v_i$ ($j \to i$ 무한 간선)
    \end{enumerate}
  ...
  \Algorithm{LP \& Flow Duality}{}{}{}{}{}
  \begin{itemize}
    \item \textbf{Difference Constraints (차이 제약 $\to$ SPFA)}
    \begin{itemize}
      \item $x_v - x_u \le w \iff u \to v$ (가중치 $w$).\\ $x_i \le C \iff 0 \to i$ ($C$), $x_i \ge C \iff i \to 0$ ($-C$).\\ $x_i - x_j = C \iff$ 양방향 ($C, -C$).
      \item 더미 노드 $0$을 시작점으로 SPFA 실행. 구한 최단 거리 dist[i] 값이 곧 제약을 만족하는 변수 $x_i$의 해(최댓값).
      \item 더미 노드 $0 \to i$ ($w=0$) 간선 필수. dist[u] == INF일 땐 relaxation 금지. cnt[u] >= V 감지 시 음수 사이클 존재 = 해 없음(모순).
    \end{itemize}

    \item \textbf{Convex Potential $\to$ MCMF Dual (포텐셜 차이 + 볼록 비용)}
    \begin{itemize}
      \item 제약 조건 $x_v - x_u \le C \implies u \to v$ (용량 $\infty$, 비용 $C$).
      \item 목표 차이 $T$ 및 오차 패널티 $|(x_v - x_u) - T|$ : 정점 $u, v$에 공급/수요 $+T$ 부여 후 $u \leftrightarrow v$ 양방향 (용량 $\infty$, 비용 $1$) 간선 생성.
      \item 일반 볼록 비용 $f(x_v - x_u)$ : (구간 길이 $\to$ 용량), (기울기 $\to$ 비용) 변환하여 $u \to v$ 다중 간선 생성.
      \item MCMF 실행 $\to$ 최종 Min Cost가 구하는 최적 비용. (포텐셜 복원 시 잔여 그래프 $h_u$ 활용)
      \item 제약 간선 비용 $C < 0$이면 일반 Dijkstra MCMF 불가 $\to$ SPFA-Dijkstra MCMF 적용, 또는 Min-Cost Circulation 사용.
    \end{itemize}

    \item \textbf{Planar Min-Cut $\to$ Dual Dijkstra (평면 그래프 민컷)}
    \begin{itemize}
      \item 면을 정점으로 변환. 원본 간선의 좌/우 면을 잇는 Dual 간선 생성 (가중치 = 원본 용량).
      \item $S_{face}$에서 $T_{face}$까지 다익스트라 실행. 최단 거리 $dist[T_face]$ = 원본 그래프의 최소컷 값.
      \item 외곽 면을 $S_{face}, T_{face}$ 2개로 분리 정의. 용량 $\infty$ 원본 간선은 Dual 간선을 생성하지 않음.
    \end{itemize}
  \end{itemize}
\end{multicols*}
