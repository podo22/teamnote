...
\begin{document} \begin{multicols*}{3}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      \item 제약 조건 $x_v - x_u \le C \implies u \to v$ (용량 $\infty$, 비용 $C$).\\ 볼록 비용 $f(x_v - x_u)$는 기울기 $w_k$(비용), 구간 $c_k$(용량)인 병렬 간선으로 분할 생성.
      \item 구축한 그래프에서 MCMF 실행. 구하고자 하는 최소 비용/목표값 = MCMF의 최종 Min Cost. (변수 $x_i$ 값 복원 필요 시 잔여 그래프 포텐셜 $h_u$ 활용)
