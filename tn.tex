...
\begin{document} \begin{multicols*}{3}
  ...
  \begin{itemize}
    ...
    \item Dilworth / Mirsky 정리\\
    - Dilworth: 최소 체인 분할 수 = 최대 반사슬 크기\\
    - Mirsky: 최소 반사슬 분할 수 = 최대 체인 크기 (= LIS 길이)\\
    - "수열을 최소 개수의 감소수열로 분할" = LIS 길이 (Mirsky)
    ...
    \item Zobrist Hashing: 각 값에 랜덤 64bit 부여, 집합/상태 해시 = XOR(개수 구분은 +). 순서 무관 비교.
    \item 가우스 소거 응용: 기대값/확률 DP에 순환이 있으면 $\to$ 연립방정식 $\to$ 가우스. 스패닝 트리 개수 = 라플라시안 cofactor det.
  \end{itemize}
